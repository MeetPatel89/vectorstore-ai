...
def test_persistence_across_client_reopen(tmp_path) -> None:
    ...
    assert reopened.get(["one"]) == [
        Chunk("one", "Persistent", {"kind": "note"})
    ]


def test_empty_metadata_is_accepted_and_returned_as_empty(tmp_path) -> None:
...
def test_multi_key_filter_uses_explicit_and(tmp_path) -> None:
