...
class TestValueObjects:
    def test_document_requires_doc_id(self):
        with pytest.raises(ValueError):
            CatalogDocument(doc_id="")

    def test_chunk_requires_nonempty_text(self):
        with pytest.raises(ValueError):
            CatalogChunk(chunk_id="c1", doc_id="d1", text="   ")

    def test_chunk_computes_content_hash_by_default(self):
        chunk = CatalogChunk(chunk_id="c1", doc_id="d1", text="hello")
        assert chunk.content_hash == content_hash("hello")

    def test_scope_rejects_empty_visibility_tuple(self):
        with pytest.raises(ValueError):
            RetrievalScope(visibility=())

    def test_scope_rejects_empty_tenant(self):
        with pytest.raises(ValueError):
            RetrievalScope(tenant_id="")

    def test_sqlite_catalog_satisfies_protocols(self):
        with SqliteDocumentCatalog() as instance:
...
class TestStructuredFind:
    ...
    def test_unsupported_operator_rejected(self, catalog):
        with pytest.raises(ValueError):
            catalog.find({"severity": {"$regex": "x"}})

    def test_attributes_round_trip(self, catalog):
    ...
    def test_upsert_updates_existing_document(self, catalog):
        catalog.upsert_documents(
            [
                CatalogDocument(
                    doc_id="INC-1104", title="updated", status="RESOLVED"
                )
            ]
        )
        (doc,) = catalog.find({"doc_id": "INC-1104"})
...
class TestScopeEnforcement:
    ...
    def test_lexical_search_respects_tenant_scope(self, catalog):
    ...
    def test_lexical_search_within_scope_finds_shared(self, catalog):
        hits = catalog.search_lexical(
...
class TestLexicalSearch:
    ...
    def test_updated_chunk_text_is_searchable(self, catalog):
        catalog.upsert_chunks(
            [
                CatalogChunk(
                    chunk_id="INC-2001:0",
    ...
    def test_hostile_query_does_not_raise_syntax_error(self, catalog):
        assert catalog.search_lexical('AND OR NOT ( " * unbalanced') == []

    def test_empty_query_rejected(self, catalog):
        with pytest.raises(ValueError):
            catalog.search_lexical("   ")

    def test_invalid_k_rejected(self, catalog):
        with pytest.raises(ValueError):
...
class TestChunksAndDeletion:
    def test_get_chunks_preserves_requested_order(self, catalog):
    ...
    def test_get_chunks_empty_input(self, catalog):
        assert catalog.get_chunks([]) == []

    def test_delete_documents_cascades(self, catalog):
        spec = EmbeddingSpec(provider="fake", model="m", dimension=8)
        catalog.mark_embedded("INC-1104:0", spec, CHUNKS[0].content_hash)
...
class TestEmbeddingLifecycleLedger:
    SPEC = EmbeddingSpec(provider="openai", model="text-embedding-3-small", dimension=1536)
    OTHER_SPEC = EmbeddingSpec(provider="st", model="all-MiniLM-L6-v2", dimension=384)

    def test_all_chunks_stale_before_embedding(self, catalog):
        assert set(catalog.stale_chunk_ids(self.SPEC)) == {
            chunk.chunk_id for chunk in CHUNKS
        }

    def test_mark_embedded_clears_staleness(self, catalog):
        for chunk in CHUNKS:
            catalog.mark_embedded(chunk.chunk_id, self.SPEC, chunk.content_hash)
        assert catalog.stale_chunk_ids(self.SPEC) == []

    def test_spaces_are_independent(self, catalog):
        for chunk in CHUNKS:
            catalog.mark_embedded(chunk.chunk_id, self.SPEC, chunk.content_hash)
        assert len(catalog.stale_chunk_ids(self.OTHER_SPEC)) == len(CHUNKS)

    def test_content_change_makes_vector_stale(self, catalog):
        catalog.mark_embedded("KB-77:0", self.SPEC, CHUNKS[2].content_hash)
        catalog.upsert_chunks(
            [
                CatalogChunk(
                    chunk_id="KB-77:0",
    ...
    def test_embedding_state_records_spec_metadata(self, catalog):
        clock = FakeClock()
        with SqliteDocumentCatalog(now=clock) as instance:
            instance.upsert_documents(INCIDENTS[:1])
            instance.upsert_chunks(CHUNKS[:1])
            instance.mark_embedded("INC-1104:0", self.SPEC, CHUNKS[0].content_hash)
    ...
    def test_embedding_state_filters_by_chunk_ids(self, catalog):
        for chunk in CHUNKS:
            catalog.mark_embedded(chunk.chunk_id, self.SPEC, chunk.content_hash)
...
class TestDurableBudgetLedger:
    ...
    def test_catalog_serves_as_router_ledger(self):
        with SqliteDocumentCatalog() as catalog:
...
class TestPersistence:
    def test_file_backed_catalog_survives_reopen(self, tmp_path):
        ...
        with SqliteDocumentCatalog(path) as catalog:
            ...
            catalog.mark_embedded("KB-77:0", spec, CHUNKS[2].content_hash)
...
class TestLexicalUnavailable:
    def test_typed_error_when_fts_missing(self):
        with SqliteDocumentCatalog() as catalog:
