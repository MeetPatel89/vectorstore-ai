...
def test_openai_embedding_batches_and_restores_response_order(monkeypatch) -> None:
    client = _FakeClient()
...
def test_openai_embedding_uses_known_model_dimension(monkeypatch) -> None:
...
def test_openai_embedding_requires_api_key(monkeypatch) -> None:
...
def test_unknown_model_requires_explicit_dimension(monkeypatch) -> None:
    monkeypatch.setattr(openai, "OpenAI", lambda **kwargs: _FakeClient())
...
def test_embedding_empty_input_does_not_call_api(monkeypatch) -> None:
    client = _FakeClient()
