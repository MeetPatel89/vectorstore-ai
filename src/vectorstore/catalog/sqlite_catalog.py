...
from ._sqlite_budget import _SqliteBudgetLedger
from .base import (
    CatalogChunk,
    CatalogDocument,
    EmbeddingState,
...
class SqliteDocumentCatalog:
    ...
    def __init__(
        ...
        self._budget = _SqliteBudgetLedger(self._connection, now)
        with self._connection:
            self._connection.executescript(_SCHEMA)
            self._budget.initialize_schema()
    ...
    def upsert_documents(self, documents: list[CatalogDocument]) -> None:
        ...
        rows = [
            (
                ...
                document.created_at,
                document.updated_at,
                json.dumps(dict(document.attributes), sort_keys=True),
            )
