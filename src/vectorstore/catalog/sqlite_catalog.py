...
from vectorstore.embeddings.pricing import (
    EmbeddingCharge,
    UsdAmount,
    nanos_to_usd,
)
...
class SqliteDocumentCatalog:
    ...
    def __init__(
        ...
        with self._connection:
            ...
            self._initialize_usage_schema()
            self._fts_available = self._try_create_fts()

    def _initialize_usage_schema(self) -> None:
        ...
        if _USAGE_COLUMNS <= columns:
            self._create_usage_indexes()
            return
        if columns == _LEGACY_USAGE_COLUMNS:
            self._migrate_legacy_usage_schema()
            return
        raise RuntimeError(
    ...
    def _create_usage_schema(self) -> None:
        self._connection.execute(_CREATE_USAGE_TABLE)
        self._create_usage_indexes()

    def _create_usage_indexes(self) -> None:
        for statement in _USAGE_INDEXES:
            self._connection.execute(statement)

    def _migrate_legacy_usage_schema(self) -> None:
    ...
    def usage_records(self) -> tuple[EmbeddingUsageRecord, ...]:
        ...
        return tuple(self._usage_record_from_row(row) for row in rows)
