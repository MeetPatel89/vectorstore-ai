"""Azure SQL persistence for the embedding budget ledger."""
...
class _AzureSqlBudgetLedger:
    """Persist atomic budget reservations through an Azure SQL boundary."""

    def __init__(
        self,
        database: _AzureSqlDatabase,
        *,
        usage_table: str,
        schema_table: str,
        schema_version: int,
    ...
    def reserve(
        ...
        *,
        ...
        """Atomically reserve predicted spend across Azure SQL sessions."""
        ...
        month_start = moment.strftime("%Y-%m-01")
        ...
        with self._database.cursor(write=True) as cursor:
            ...
            if (
                ...
                and self._sum_active_nanos(cursor, month_start=month_start)
                + charge.charge_nanos
                > monthly_limit_nanos
            ):
...
FROM {self._usage_table} WITH (UPDLOCK, HOLDLOCK)
WHERE [event_id] = ?
""".strip()
        with self._database.cursor(write=True) as cursor:
...
SET [tokens] = ?, [rate_nanos_per_million] = ?, [price_version] = ?,
    [charge_nanos] = ?, [status] = ?, [expires_at] = NULL, [updated_at] = ?
WHERE [event_id] = ?
""".strip(),
                (
    ...
    def release(self, reservation: BudgetReservation) -> None:
        ...
        with self._database.cursor(write=True) as cursor:
            cursor.execute(
                ...
                "WITH (UPDLOCK, HOLDLOCK) WHERE [event_id] = ?",
                (reservation.reservation_id,),
            )
            row = cursor.fetchone()
            if row is None:
    ...
    def tokens_today(self, provider: str, model: str | None = None) -> int:
        ...
        with self._database.cursor() as cursor:
            ...
            row = cursor.fetchone()
        if row is None:
            raise RuntimeError("Azure SQL token total query returned no row")
    ...
    def usage_records(self) -> tuple[EmbeddingUsageRecord, ...]:
        ...
        moment = self._now()
        fields = ", ".join(f"[{field}]" for field in _USAGE_FIELDS)
        with self._database.cursor(write=True) as cursor:
            self._expire_reservations(cursor, moment)
            cursor.execute(
...
""".strip(),
    ...
    def _sum_active_nanos(
        ...
        *,
        ...
        if (day is None) == (month_start is None):
            raise ValueError("exactly one of day or month_start must be supplied")
        parameters: list[object]
        if day is not None:
            date_clause = "[date] = ?"
            parameters = [day]
        else:
            date_clause = "[date] >= ? AND [date] < DATEADD(month, 1, ?)"
            parameters = [month_start, month_start]
...
WHERE {date_clause} AND [status] IN (?, ?)
""".strip(),
            tuple(parameters),
        )
        row = cursor.fetchone()
        if row is None:
            raise RuntimeError("Azure SQL spend total query returned no row")
        return int(_row_value(row, 0, "total"))
